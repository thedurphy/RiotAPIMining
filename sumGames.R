sumGames <- function(ids, custom.games = F, delay = 0) {
require(rjson)
## Primary Code
summonerGames <- function(x){
...
game.count <- length(df$games)
summonerGame <- data.frame(t(sapply(1:game.count, function(x){
						data.frame("summonerId" = df$summonerId,
                       "teamId" = df$games[[x]]$teamId,
                       "championId" = df$games[[x]]$championId)
})))	
## Code to determine game type
gameType <- sapply(1:game.count, function(x){df$games[[x]]$gameType})
...
results <- as.character(gamesAlmost$results)
...
raw.Frames <- cbind(unlist(output$gameId), unlist(output$summonerId), unlist(output$Summoner), unlist(output$teamId), unlist(output$championId), unlist(output$ch.Names), unlist(output$results))
row.names(raw.Frames) <- NULL
sub.Frames <- data.frame(raw.Frames, stringsAsFactors = F)
names(sub.Frames) <- c("GameID", "SummonerID", "Summoner", "TeamID", "ChampionID", "Champion.Names", "Outcome")
sub.Frames						
}

## Data Arrangement
all.Frames <- lapply(ids, function(x){
	tryCatch(summonerGames(x), error=function(e) NULL)
	Sys.sleep(delay)  ## Delays the running of the next iteration of the code
	})

GameID <- unlist(sapply(1:length(ids), function(x){
    all.Frames[[x]]$GameID
}))	
	
SummonerID <- unlist(sapply(1:length(ids), function(x){
    all.Frames[[x]]$SummonerID
}))	

Summoner <- unlist(sapply(1:length(ids), function(x){
    all.Frames[[x]]$Summoner
}))

TeamID <- unlist(sapply(1:length(ids), function(x){
    all.Frames[[x]]$TeamID
}))

ChampionID <- unlist(sapply(1:length(ids), function(x){
    all.Frames[[x]]$ChampionID
}))

Champion.Names <- unlist(sapply(1:length(ids), function(x){
    all.Frames[[x]]$Champion.Names
}))

Outcome <- unlist(sapply(1:length(ids), function(x){
    all.Frames[[x]]$Outcome
}))



beta.Frames <- cbind(GameID, SummonerID, Summoner, TeamID, ChampionID, Champion.Names, Outcome)
beta.Frames <- data.frame(beta.Frames)
beta.Frames$GameID <- as.numeric(as.character(beta.Frames$GameID)); beta.Frames$SummonerID <- as.numeric(as.character(beta.Frames$SummonerID));
beta.Frames$TeamID <- as.numeric(as.character(beta.Frames$TeamID)); beta.Frames$ChampionID <- as.numeric(as.character(beta.Frames$ChampionID))

final.Frame <- unique(beta.Frames)
row.names(final.Frame) <- NULL
